...
//		System.out.println("계산금액 = " + totalPrice + " 실제 계산해야할 금액 = " + orderPriceCheck );
	...
	/*
	 * @GetMapping({"/orderList", "/orderList/{page}"}) public String
	 * orderList(@AuthenticationPrincipal LoginService user, Model
	 * model, @PathVariable(required = false) Integer page) { if (user == null) { //
	 * System.out.println("주문목록 비로그인"); } else { long userId =
	 * user.getUser().getId();
	 * 
	 * Page p = new Page(page); List<OrderList> orderList =
	 * orderService.orderList(userId, p);
	 * 
	 * if (orderList.size() == 0) { return "order/orderList"; }
	 * 
	 ...
	 * model.addAttribute("page", p); model.addAttribute("user", user.getUser());
	 * model.addAttribute("cartList", cartList); model.addAttribute("orderList",
	 * orderList); }
	 * 
